...
LOG_FILE = "results/causal_propaganda_log.md"
...
# Try encoding with a space to match the prompt context
target_ids = tokenizer.encode(" " + target_str, add_special_tokens=False)
log(f"Searching for target IDs: {target_ids} (representing ' {target_str}')")
...
if start_idx == -1:
    # Fallback: search for just the word part
    target_ids = tokenizer.encode(target_str, add_special_tokens=False)
    log(f"Fallback: Searching for target IDs: {target_ids} (representing '{target_str}')")
    for i in range(len(prompt_ids) - len(target_ids) + 1):
        if prompt_ids[i:i+len(target_ids)] == target_ids:
            start_idx = i
            break
...
if start_idx != -1:
    ...
    log("Error: Could not locate target tokens in prompt.")
