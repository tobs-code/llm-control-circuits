...
def load_type_vectors(vector_type):
    """Load a specific vector type (refusal, propaganda, golden_gate) across all models."""
    vectors = {}
    model_prefixes = ["qwen25", "internlm2", "deepseek_r1"]
    for prefix in model_prefixes:
        key = f"{prefix}_{vector_type}"
        path = os.path.join(VECTOR_DIR, f"{key}_vectors.pt")
        if os.path.exists(path):
            vectors[prefix] = torch.load(path, map_location='cpu')
        else:
...
def analyze_vector_type(vector_type):
    vectors = load_type_vectors(vector_type)
    if len(vectors) < 2:
        print(f"Skipping {vector_type}: insufficient models")
        return None
    ...
    if not common_layers:
        print(f"Skipping {vector_type}: no common layers")
        return None
...
def save_results(results):
    ...
    tag = f"{results['vector_type']}_cross_model"
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(f"# Cross-Model Vector Geometry Analysis for {results['vector_type']} vectors\n\n")
        f.write(f"**Run:** {results['timestamp']}\n\n")
        f.write(f"**Models:** {', '.join(results['models'])}\n\n")
        ...
        for layer in sorted(results["pairwise_analysis"].keys(), key=int):
            ...
            if layer_data["norms"]:
                ...
                f.write("| Model | Norm |\n")
                f.write("|-------|------|\n")
                for model_name, norm in sorted(layer_data["norms"].items()):
                    f.write(f"| {model_name} | {norm:.4f} |\n")
                f.write("\n")
            if layer_data["cosine_similarities"]:
                f.write("**Cosine Similarities:**\n")
                f.write("| Model Pair | Cosine Similarity |\n")
                f.write("|------------|-------------------|\n")
            ...
            if layer_data["orthogonality_scores"]:
                ...
                f.write("| Model Pair | Orthogonality |\n")
                f.write("|------------|---------------|\n")
                for pair, ortho in sorted(layer_data["orthogonality_scores"].items()):
                    f.write(f"| {pair} | {ortho:.4f} |\n")
                f.write("\n")
            f.write("---\n\n")
    print(f"Saved results for {results['vector_type']} cross-model to:\n  {md_path}\n  {json_path}")

def main():
    vector_types = ["refusal", "propaganda", "golden_gate"]
    for vtype in vector_types:
        print(f"Analyzing {vtype} vectors across models...")
        res = analyze_vector_type(vtype)
        if res:
            save_results(res)
        else:
            print(f"  Skipped {vtype}")
    print("\nDone.")
