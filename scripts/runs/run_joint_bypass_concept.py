...
PROMPTS = [
    ...
    ("Landmark_EN", "Name a famous landmark and describe it in one sentence."),
    (
        "Introspection_EN",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run joint political bypass + concept steering test.")
    parser.add_argument("--model", default=MODEL_ID)
    parser.add_argument("--tag", default=None)
    parser.add_argument("--concept-alpha", type=float, default=CONCEPT_ALPHA)
...
def main():
    ...
    tag = args.tag or slugify(f"{args.model}_joint_bypass_concept")

    os.makedirs(RESULTS_DIR, exist_ok=True)
    md_path = os.path.join(RESULTS_DIR, f"joint_bypass_concept_{tag}.md")
    json_path = os.path.join(RESULTS_DIR, f"joint_bypass_concept_{tag}.json")
    ...
    log(f"# Joint Bypass + Concept Steering ({args.model})")
    log(f"Run: {datetime.now().isoformat(timespec='seconds')}")
    log(f"Concept alpha: `{args.concept_alpha}`")
    ...
    def concept_hook(vec):
        def hook(_module, _input, output):
            is_tuple = isinstance(output, tuple)
            val = output[0] if is_tuple else output
            patched = val.clone()
            patched[:, -1, :] = patched[:, -1, :] + args.concept_alpha * vec
    ...
    conditions = [
        ...
        ("bypass_only", True, False),
        ("concept_only", False, True),
        ("bypass_plus_concept", True, True),
    ]

    rows = []
    for label, use_bypass, use_concept in conditions:
        log(f"## {label}")
        handles = []
        if use_bypass:
            for layer_idx in steering_layers:
            ...
            for layer_idx in CONCEPT_LAYERS:
                handles.append(
                    get_attn_module(layers[layer_idx]).register_forward_hook(
                        concept_hook(vec_concept[layer_idx])
                    )
                )
        try:
